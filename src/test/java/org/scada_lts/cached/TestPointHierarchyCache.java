...
public class TestPointHierarchyCache {
	...
	public void addPointAndFolderAndPointMoveToFolder() throws Exception {
		...
		Assert.assertEquals(true,  contain1 );
		...
		Assert.assertEquals(true,  contain2 );
	...
	public void addFolders() throws Exception {
		...
		Assert.assertEquals(true,  contain3 );
		...
		Assert.assertEquals(true,  contain4 );
	...
	public void addPointsInFolder() throws Exception {
		...
		Assert.assertEquals(true,  contain5 );
		...
		Assert.assertEquals(true,  contain6 );
	...
	public void delete1() throws Exception {
		...
		Assert.assertEquals(false, containInCache(cache.getOnBaseParentId(keyFolder3), keyPoint6, PointHierarchyCache.IS_NOT_FOLDER));
		cache.deletePoint(keyFolder5, keyPoint5);
		Assert.assertEquals(false, containInCache(cache.getOnBaseParentId(keyFolder5), keyPoint5, PointHierarchyCache.IS_NOT_FOLDER));
		
		Assert.assertEquals(true, containInCache(cache.getOnBaseParentId(PointHierarchyCache.ROOT), keyPoint5, PointHierarchyCache.IS_NOT_FOLDER));
		Assert.assertEquals(true, containInCache(cache.getOnBaseParentId(PointHierarchyCache.ROOT), keyPoint6, PointHierarchyCache.IS_NOT_FOLDER));
	}
	...
	public void delete2() throws Exception {
		...
		Assert.assertEquals(false, containInCache(cache.getOnBaseParentId(PointHierarchyCache.ROOT), keyFolder5, PointHierarchyCache.IS_FOLDER));
		
		cache.deleteFolder(PointHierarchyCache.ROOT, keyFolder1);
		Assert.assertEquals(false, containInCache(cache.getOnBaseParentId(PointHierarchyCache.ROOT), keyFolder1, PointHierarchyCache.IS_FOLDER));
		
		Assert.assertEquals(true, containInCache(cache.getOnBaseParentId(PointHierarchyCache.ROOT), keyPoint5, PointHierarchyCache.IS_NOT_FOLDER));
		Assert.assertEquals(true, containInCache(cache.getOnBaseParentId(keyFolder3), keyPoint6, PointHierarchyCache.IS_NOT_FOLDER));
		Assert.assertEquals(true, containInCache(cache.getOnBaseParentId(keyFolder3), keyPoint7, PointHierarchyCache.IS_NOT_FOLDER));
	}
	...
	public void delete3() throws Exception {
		...
		cache.printTreeInCash("", 0);
		Assert.assertEquals(false, containInCache(cache.getOnBaseParentId(keyFolder2), keyFolder3, PointHierarchyCache.IS_FOLDER));
		Assert.assertEquals(true, containInCache(cache.getOnBaseParentId(PointHierarchyCache.ROOT), keyPoint6, PointHierarchyCache.IS_NOT_FOLDER));
		Assert.assertEquals(true, containInCache(cache.getOnBaseParentId(PointHierarchyCache.ROOT), keyPoint7, PointHierarchyCache.IS_NOT_FOLDER));
	}
	...
	public void edit() throws Exception {
		...
		cache.printTreeInCash("", 0);
		Assert.assertEquals("newTitle1", findFolder(cache.getOnBaseParentId(PointHierarchyCache.ROOT), keyFolder1).getTitle());
		Assert.assertEquals("newTitle2", findFolder(cache.getOnBaseParentId(keyFolder1), keyFolder2).getTitle());
		Assert.assertEquals("newTitle3", findFolder(cache.getOnBaseParentId(keyFolder2), keyFolder3).getTitle());
		Assert.assertEquals("newTitle5", findFolder(cache.getOnBaseParentId(PointHierarchyCache.ROOT), keyFolder5).getTitle());
	}
	...
	public void getOnBaseName() throws Exception {
		...
		Assert.assertEquals(true, containInCache(result, keyFolder1, PointHierarchyCache.IS_FOLDER ));
		Assert.assertEquals(false, containInCache(result, keyFolder2, PointHierarchyCache.IS_FOLDER ));
		Assert.assertEquals(true, containInCache(result, keyFolder5, PointHierarchyCache.IS_FOLDER ));
		Assert.assertEquals(true, containInCache(result, keyPoint1, PointHierarchyCache.IS_NOT_FOLDER));
		Assert.assertEquals(false, containInCache(result, keyFolder4, PointHierarchyCache.IS_FOLDER));
		Assert.assertEquals(false, containInCache(result, keyFolder3, PointHierarchyCache.IS_FOLDER));
	...
	public void getPaths() throws Exception {
		...
		Assert.assertEquals(true, okResultForFolder1);
		...
		Assert.assertEquals(true, (resultFolder2.size()==2) && (resultFolder2.get(0).getTitle()=="hlkjhd"));
		
		List<PointHierarchyNode> resultForFolder5 = cache.getPaths(keyFolder5, PointHierarchyCache.IS_FOLDER);
		
		// Because return himself and parent folder when size = 2
		Assert.assertEquals(true, (resultForFolder5.size()==2) && (resultForFolder5.get(0).getTitle()=="test"));
		
		List<PointHierarchyNode> resultkeyPoint1 = cache.getPaths(keyPoint1, PointHierarchyCache.IS_NOT_FOLDER);
		
		Assert.assertEquals(true, (resultkeyPoint1.size()==2) && (resultkeyPoint1.get(0).getTitle()=="test"));
		
		List<PointHierarchyNode> resultkeyFolder4 = cache.getPaths(keyFolder4, PointHierarchyCache.IS_FOLDER);
		
		Assert.assertEquals(true, (resultkeyFolder4.size()==1) && (resultkeyFolder4.get(0).getTitle()=="hlkjh"));
		
		List<PointHierarchyNode> resultkeyFolder3 = cache.getPaths(keyFolder3, PointHierarchyCache.IS_FOLDER);
		
		Assert.assertEquals(true, (resultkeyFolder3.size()==1) && (resultkeyFolder3.get(0).getTitle()=="lkjh"));
