...
public class IpAdresUtilsTest {
    ...
    public void ipWhiteListCheck() throws Exception {
        ...
        Assert.assertEquals(IpAddressUtils.ipWhiteListCheck(allowedIps, correctIp1) , IpAdresUtils.ipWhiteListCheck(allowedIps, correctIp1));
        Assert.assertEquals(IpAddressUtils.ipWhiteListCheck(allowedIps, correctIp2) , IpAdresUtils.ipWhiteListCheck(allowedIps, correctIp2));

        Assert.assertEquals(IpAddressUtils.ipWhiteListCheck(allowedIps, incorrectIp) , IpAdresUtils.ipWhiteListCheck(allowedIps, incorrectIp));
    }
...
//        System.out.println(IpAddressUtils.checkIpMask(ip5));
        ...
        Assert.assertEquals(IpAddressUtils.checkIpMask(ip1) , IpAdresUtils.checkIpMask(ip1));
        Assert.assertEquals(IpAddressUtils.checkIpMask(ip2) , IpAdresUtils.checkIpMask(ip2));
        Assert.assertEquals(IpAddressUtils.checkIpMask(ip3) , IpAdresUtils.checkIpMask(ip3));
        Assert.assertEquals(IpAddressUtils.checkIpMask(ip5) , IpAdresUtils.checkIpMask(ip5));
        //   assertEquals(IpAddressUtils.checkIpMask(ip4) , IpAdresUtils.checkIpMask(ip4));

        Assert.assertEquals(IpAdresUtils.checkIpMask(ip4),"Incorrect value in IP address" );
    }
    ...
    public void toIpAddress() throws Exception {
        ...
        Assert.assertEquals(Arrays.toString(IpAdresUtils.toIpAddress(ip1)), Arrays.toString(IpAddressUtils.toIpAddress(ip1)));
        Assert.assertEquals(Arrays.toString(IpAdresUtils.toIpAddress(ip2)), Arrays.toString(IpAddressUtils.toIpAddress(ip2)));
        Assert.assertEquals(Arrays.toString(IpAdresUtils.toIpAddress(ip3)), Arrays.toString(IpAddressUtils.toIpAddress(ip3)));
