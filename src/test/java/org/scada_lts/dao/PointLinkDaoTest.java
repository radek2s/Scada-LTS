...
public class PointLinkDaoTest extends TestDAO {
	...
	public void test() {
		...
		Assert.assertTrue(pointLinkSelectId.getId() == firstId);
		Assert.assertTrue(pointLinkSelectId.getXid().equals(XID));
		Assert.assertTrue(pointLinkSelectId.getSourcePointId() == SOURCE_POINT_ID);
		Assert.assertTrue(pointLinkSelectId.getTargetPointId() == TARGET_POINT_ID);
		Assert.assertTrue(pointLinkSelectId.getScript().equals(SCRIPT));
		Assert.assertTrue(pointLinkSelectId.getEvent() == EVENT_TYPE);
		Assert.assertTrue(pointLinkSelectId.isDisabled() == DISABLED);

		//Select single object by xid
		PointLinkVO pointLinkSelectXid = pointLinkDAO.getPointLink(XID);
		Assert.assertTrue(pointLinkSelectXid.getId() == firstId);
		Assert.assertTrue(pointLinkSelectXid.getXid().equals(XID));
		Assert.assertTrue(pointLinkSelectXid.getSourcePointId() == SOURCE_POINT_ID);
		Assert.assertTrue(pointLinkSelectXid.getTargetPointId() == TARGET_POINT_ID);
		Assert.assertTrue(pointLinkSelectXid.getScript().equals(SCRIPT));
		Assert.assertTrue(pointLinkSelectXid.getEvent() == EVENT_TYPE);
		Assert.assertTrue(pointLinkSelectXid.isDisabled() == DISABLED);

		//Select all objects
		List<PointLinkVO> pointLinkList = pointLinkDAO.getPointLinks();
		//Check list size
		Assert.assertTrue(pointLinkList.size() == LIST_SIZE);
		//Check IDs
		Assert.assertTrue(pointLinkList.get(0).getId() == firstId);
		Assert.assertTrue(pointLinkList.get(1).getId() == secondId);
		...
		Assert.assertTrue(pointLinkDAO.getPointLink(firstId) == null);
	}
