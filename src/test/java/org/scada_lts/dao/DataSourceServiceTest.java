...
public class DataSourceServiceTest extends TestDAO {
	...
	public void testCRUD() {
		...
		Assert.assertTrue(dataSourceService.getDataSources().size() == 1);
		...
		Assert.assertTrue(dataSourceUpdate.getId() == dataSource.getId());
		Assert.assertTrue(dataSourceUpdate.getName().equals(UPDATE_NAME));
		Assert.assertTrue(dataSourceUpdate.isEnabled() == UPDATE_ENABLE);

		//Delete
		dataSourceService.deleteDataSource(dataSourceUpdate.getId());
		Assert.assertTrue(dataSourceService.getDataSources().size() == 0);
	}
	...
	public void testCopyMethods() {
		...
		Assert.assertTrue(dataSourceList.size() == 2);
		Assert.assertTrue(dataSourceList.get(0).getTypeKey().equals(dataSourceList.get(1).getTypeKey()));
	}
}
