...
public class DataSourceDaoTest extends TestDAO {
	...
	public void testDataSource() {
		...
		Assert.assertTrue(dataSourceSelectId.getXid().equals(dataSource.getXid()));
		Assert.assertTrue(dataSourceSelectId.getName().equals(dataSource.getName()));
		Assert.assertTrue(dataSourceSelectId.isEnabled() == dataSource.isEnabled());

		DataSourceVO dataSourceSelectXid = dataSourceDAO.getDataSource(secondId);
		Assert.assertTrue(dataSourceSelectXid.getXid().equals(secondDataSource.getXid()));
		Assert.assertTrue(dataSourceSelectXid.getName().equals(secondDataSource.getName()));
		Assert.assertTrue(dataSourceSelectXid.isEnabled() == secondDataSource.isEnabled());

		//Select all objects
		List<DataSourceVO<?>> dataSourceList = dataSourceDAO.getDataSources();
		//Check list size
		Assert.assertTrue(dataSourceList.size() == LIST_SIZE);
		//Check IDs
		Assert.assertTrue(dataSourceList.get(0).getId() == dataSource.getId());
		Assert.assertTrue(dataSourceList.get(1).getId() == secondDataSource.getId());
		...
		Assert.assertTrue(dataSourceSelectUpdate.getId() == dataSource.getId());
		Assert.assertTrue(dataSourceSelectUpdate.getXid().equals(UPDATE_XID));
		Assert.assertTrue(dataSourceSelectUpdate.getName().equals(UPDATE_NAME));
		Assert.assertTrue(dataSourceSelectUpdate.isEnabled() == UPDATE_ENABLE);
