...
public class ScheduledEventDaoTest extends TestDAO {
	...
	public void test() {
		...
		Assert.assertTrue(scheduledEventVoSelect.getId() == firstId);
		Assert.assertTrue(scheduledEventVoSelect.getXid().equals(XID));
		Assert.assertTrue(scheduledEventVoSelect.getAlias().equals(ALIAS));
		Assert.assertTrue(scheduledEventVoSelect.getAlarmLevel() == ALARM_LEVEL);
		Assert.assertTrue(scheduledEventVoSelect.getScheduleType() == SCHEDULE_TYPE);
		Assert.assertTrue(scheduledEventVoSelect.isReturnToNormal() == RETURN_TO_NORMAL);
		Assert.assertTrue(scheduledEventVoSelect.isDisabled() == DISABLED);
		Assert.assertTrue(scheduledEventVoSelect.getActiveYear() == ACTIVE_YEAR);
		Assert.assertTrue(scheduledEventVoSelect.getActiveMonth() == ACTIVE_MONTH);
		Assert.assertTrue(scheduledEventVoSelect.getActiveDay() == ACTIVE_DAY);
		Assert.assertTrue(scheduledEventVoSelect.getActiveHour() == ACTIVE_HOUR);
		Assert.assertTrue(scheduledEventVoSelect.getActiveMinute() == ACTIVE_MINUTE);
		Assert.assertTrue(scheduledEventVoSelect.getActiveSecond() == ACTIVE_SECOND);
		Assert.assertTrue(scheduledEventVoSelect.getActiveCron().equals(ACTIVE_CRON));
		Assert.assertTrue(scheduledEventVoSelect.getInactiveYear() == INACTIVE_YEAR);
		Assert.assertTrue(scheduledEventVoSelect.getInactiveMonth() == INACTIVE_MONTH);
		Assert.assertTrue(scheduledEventVoSelect.getInactiveDay() == INACTIVE_DAY);
		Assert.assertTrue(scheduledEventVoSelect.getInactiveHour() == INACTIVE_HOUR);
		Assert.assertTrue(scheduledEventVoSelect.getInactiveMinute() == INACTIVE_MINUTE);
		Assert.assertTrue(scheduledEventVoSelect.getInactiveSecond() == INACTIVE_SECOND);
		Assert.assertTrue(scheduledEventVoSelect.getInactiveCron().equals(INACTIVE_CRON));

		//Select all objects
		List<ScheduledEventVO> scheduledEventVOList = scheduledEventDAO.getScheduledEvents();
		//Check list size
		Assert.assertTrue(scheduledEventVOList.size() == LIST_SIZE);
		//Check IDs
		Assert.assertTrue(scheduledEventVOList.get(0).getId() == firstId);
		Assert.assertTrue(scheduledEventVOList.get(1).getId() == secondId);
