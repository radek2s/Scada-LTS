...
public class MailingListDaoTest extends TestDAO {
	...
	public void test() {
		...
		Assert.assertTrue(mailingListSelectId.getId() == firstId);
		Assert.assertTrue(mailingListSelectId.getXid().equals(XID));
		Assert.assertTrue(mailingListSelectId.getName().equals(NAME));
		MailingList mailingListSelectXid = mailingListDAO.getMailingList(secondMailingList.getXid());
		Assert.assertTrue(mailingListSelectXid.getId() == secondId);
		Assert.assertTrue(mailingListSelectXid.getXid().equals(SECOND_XID));
		Assert.assertTrue(mailingListSelectXid.getName().equals(SECOND_NAME));

		//Select all objects
		List<MailingList> mailingLists = mailingListDAO.getMailingLists();
		//Check list size
		Assert.assertTrue(mailingLists.size() == LIST_SIZE);
		//Check IDs
		Assert.assertTrue(mailingLists.get(0).getId() == firstId);
		Assert.assertTrue(mailingLists.get(1).getId() == secondId);
		...
		Assert.assertTrue(mailingListSelectUpdate.getId() == mailingListUpdate.getId());
		Assert.assertTrue(mailingListSelectUpdate.getXid().equals(mailingListUpdate.getXid()));
		Assert.assertTrue(mailingListSelectUpdate.getName().equals(mailingListUpdate.getName()));
