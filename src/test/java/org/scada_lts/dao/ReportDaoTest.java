...
public class ReportDaoTest extends TestDAO {
	...
	public void test() {
		...
		Assert.assertTrue(reportSelect.getId() == firstId);
		Assert.assertTrue(reportSelect.getName().equals(NAME));
		Assert.assertTrue(reportSelect.getUserId() == USER_ID);

		//Select all objects
		List<ReportVO> reportListAll = reportDAO.getReports();
		//Check size
		Assert.assertTrue(reportListAll.size() == LIST_SIZE);
		//Check IDs
		Assert.assertTrue(reportListAll.get(0).getId() == firstId);
		Assert.assertTrue(reportListAll.get(1).getId() == secondId);

		//Select all objects which contain specific userId
		List<ReportVO> reportListId = reportDAO.getReports(report.getUserId());
		//Check size
		Assert.assertTrue(reportListId.size() == LIST_SIZE);
		//Check ID
		Assert.assertTrue(reportListId.get(0).getId() == firstId);
		...
		Assert.assertTrue(reportSelectUpdate.getId() == report.getId());
		Assert.assertTrue(reportSelectUpdate.getUserId() == USER_ID);
		Assert.assertTrue(reportSelectUpdate.getName().equals(UPDATE_NAME));

		//Delete
		reportDAO.delete(firstId);
		Assert.assertTrue(reportDAO.getReport(firstId) == null);
	}
}
