...
public class DataPointDaoTest extends TestDAO {
	...
	public void test() {
		...
		DataPointVO secondDataPoint = new DataPointVO();
		...
		Assert.assertTrue(dataPointSelectId.getId() == firstId);
		Assert.assertTrue(dataPointSelectId.getXid().equals(XID));
		Assert.assertTrue(dataPointSelectId.getDataSourceId() == DATA_SOURCE_ID);

		//Select single object by xid
		DataPointVO dataPointSelectXid = dataPointDAO.getDataPoint(secondDataPoint.getXid());
		Assert.assertTrue(dataPointSelectXid.getId() == secondId);
		Assert.assertTrue(dataPointSelectXid.getXid().equals(SECOND_XID));
		Assert.assertTrue(dataPointSelectXid.getDataSourceId() == SECOND_DATA_SOURCE_ID);

		//Select all objects
		List<DataPointVO> dataPointList = dataPointDAO.getDataPoints();
		//Check list size
		Assert.assertTrue(dataPointList.size() == LIST_SIZE);
		//Check IDs
		Assert.assertTrue(dataPointList.get(0).getId() == firstId);
		Assert.assertTrue(dataPointList.get(1).getId() == secondId);

		//Update
		DataPointVO dataPointUpdate = new DataPointVO();
		...
		Assert.assertTrue(dataPointUpdate.getId() == firstId);
		Assert.assertTrue(dataPointUpdate.getXid().equals(UPDATE_XID));
