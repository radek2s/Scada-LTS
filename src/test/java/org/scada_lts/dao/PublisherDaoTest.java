...
public class PublisherDaoTest extends TestDAO {
	...
	public void test() {
		...
		Assert.assertTrue(httpSenderSelectId.getId() == firstId);
		Assert.assertTrue(httpSenderSelectId.getXid().equals(XID));
		Assert.assertTrue(httpSenderSelectId.getName().equals(NAME));
		HttpSenderVO httpSenderSelectXid = (HttpSenderVO) publisherDAO.getPublisher(secondHttpSender.getXid());
		Assert.assertTrue(httpSenderSelectXid.getId() == secondId);
		Assert.assertTrue(httpSenderSelectXid.getXid().equals(SECOND_XID));
		Assert.assertTrue(httpSenderSelectXid.getName().equals(SECOND_NAME));

		//Select all objects
		List<PublisherVO<? extends PublishedPointVO>> publisherList = publisherDAO.getPublishers();
		//Check list size
		Assert.assertTrue(publisherList.size() == LIST_SIZE);
		//Check IDs
		Assert.assertTrue(publisherList.get(0).getId() == firstId);
		Assert.assertTrue(publisherList.get(1).getId() == secondId);
		...
		Assert.assertTrue(httpSenderSelectUpdate.getId() == httpSenderSelectId.getId());
		Assert.assertTrue(httpSenderSelectUpdate.getXid().equals(UPDATE_XID));
		Assert.assertTrue(httpSenderSelectUpdate.getName().equals(UPDATE_NAME));

		//Delete
		publisherDAO.delete(firstId);
		Assert.assertTrue(publisherDAO.getPublisher(firstId) == null);
	}
}
