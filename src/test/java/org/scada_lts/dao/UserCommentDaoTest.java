...
public class UserCommentDaoTest extends TestDAO {
	...
	public void Test() {
		...
		//Check list size
		Assert.assertTrue(userCommentList.size() == LIST_SIZE);
		//Check IDs (after update)
		Assert.assertTrue(userCommentList.get(0).getUserId() == secondId);
		Assert.assertTrue(userCommentList.get(1).getUserId() == 0);
		...
		Assert.assertTrue(nextUserCommentList.size() == 0);
	}
}
